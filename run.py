...
"""
    初始化参数说明：
    model_name: HuggingFace上的模型名称，默认为MACLAB-HFUT/Psyche-R1
...
class PsycheR1Chat:
    ...
    def chat(self, system_prompt: Optional[str] = None):
        ...
        if self.local_rank in [-1, 0]:
            print("欢迎使用中文心理推理大模型Psyche-R1！输入'quit'或'exit'可结束对话。")
            if system_prompt:
                print(f"\n系统提示词: {system_prompt}\n")
            ...
            while True:
                ...
                if user_input.lower() in ['quit', 'exit']:
                    print("\n感谢使用中文心理推理大模型Psyche-R1，祝您生活愉快，再见！")
                    break
...
def main():
    ...
    try:
        ...
        print(f"启动Psyche-R1时发生错误: {str(e)}")
